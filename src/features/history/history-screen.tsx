...
function EditMovementModal({ accountOptions, item, onClose }: { accountOptions: HistoryViewModel['accountOptions']; item: HistoryItem; onClose: () => void }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="card-arca max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto p-5 sm:p-6">
        ...
        <form className="mt-5 grid gap-3" onSubmit={(event) => {
          ...
          startTransition(async () => {
            ...
            try {
              await updateManualTransaction({ id: item.id, concept: String(form.get('concept') ?? ''), amount: Number(form.get('amount') ?? 0), category: String(form.get('category') ?? ''), unit: String(form.get('unit') ?? ''), date: String(form.get('date') ?? ''), accountId: String(form.get('accountId') ?? '') });
              router.refresh();
              onClose();
            } catch (error) {
          ...
          {isLinkedMovement ? <p className="rounded-xl border border-arca-accent/25 bg-arca-accent/10 px-3 py-2 text-xs leading-relaxed text-arca-text-secondary">Este movimiento proviene de otro flujo. Puedes corregir la cuenta, mientras sus demás datos permanecen protegidos.</p> : null}
          <Field label="Concepto"><input name="concept" readOnly={isLinkedMovement} defaultValue={item.concept} className={`${CONTROL_CLASS} ${lockedFieldClass}`} /></Field>
          <Field label="Valor"><input name="amount" readOnly={isLinkedMovement} type="number" min="0" step="1" defaultValue={item.amount} className={`${CONTROL_CLASS} ${lockedFieldClass}`} /></Field>
          <Field label="Categoría"><input name="category" readOnly={isLinkedMovement} defaultValue={item.category} className={`${CONTROL_CLASS} ${lockedFieldClass}`} /></Field>
          <Field label="Unidad"><input name="unit" readOnly={isLinkedMovement} defaultValue={item.unit} className={`${CONTROL_CLASS} ${lockedFieldClass}`} /></Field>
          <Field label="Cuenta o banco"><select name="accountId" required defaultValue={item.accountId ?? ''} className={CONTROL_CLASS}><option value="" disabled>Selecciona una cuenta</option>{accountOptions.map((account) => <option key={account.id} value={account.id}>{account.label}</option>)}</select></Field>
          <Field label="Fecha"><input name="date" readOnly={isLinkedMovement} type="date" defaultValue={item.dateInputValue} className={`${CONTROL_CLASS} ${lockedFieldClass}`} /></Field>
