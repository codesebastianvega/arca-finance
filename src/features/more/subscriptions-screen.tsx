      ...
      <header className="flex items-center justify-between mb-2">
        ...
        {onNavigateToRegister && (
          <button 
            ...
            className="w-10 h-10 rounded-full bg-arca-accent text-white flex items-center justify-center shadow-lg mr-12"
          >
            <Plus size={20} />
          </button>
        )}
          ...
          <>
              ...
              if (isEditing) {
                return (
                  <div key={sub.id} className="card-arca p-4 flex flex-col gap-4 border-arca-accent/30 border">
                    ...
                    <p className="text-[10px] text-arca-text-secondary text-center px-4 mt-2">Para cambiar fechas o configuración avanzada, finaliza este registro y crea uno nuevo.</p>
                  </div>
                );
              }
            ...
            {onNavigateToRegister && (
              <button 
                ...
                className="mt-4 w-full py-4 rounded-xl bg-arca-surface-2 border border-arca-border border-dashed text-arca-text-primary font-bold text-sm flex items-center justify-center gap-2 hover:bg-arca-border transition-colors"
              >
                <Plus size={16} /> Agregar {activeTab === 'incomes' ? 'contrato' : 'suscripción'}
              </button>
            )}
          </>
        )}
